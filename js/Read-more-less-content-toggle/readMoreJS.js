    ...
    RM.init = function(options) {
        ...
        for (i = 0; i < targetLen; i++) {
            ...
            if (options.numOfWords < targetContentWords - 1) {
                ...
                moreContainer.style.cssText = 'text-align: center; margin-top: 10px; text-indent: 0'

                moreContainer.innerHTML = '<a id="rm-more_' + i + '"' // Create the More/Less link.
                    +
                    ' class="' + options.linkClass + ' btn btn-link btn-default"' +
                    ' style="cursor:pointer;" data-readmore="anchor">' + '<i class="fa fa-book" aria-hidden="true"></i>' +
                    options.moreLink +
                    '</a>';
        ...
        for (j = 0, l = rmLink.length; j < l; j++) {
            rmLink[j].onclick = function() {
                ...
                if (this.getAttribute('data-clicked') !== 'true') {
                    ...
                    this.innerHTML = '<i class="fa fa-book" aria-hidden="true"></i>' + options.moreLink;
